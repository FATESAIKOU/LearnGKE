...
| 步驟 | 項目 | 說明 | 狀態 |
|------|------|------|------|
| B4-1 | 實作 index action | 列出所有 Todos | ⬜ 未開始 |
| B4-2 | 實作搜尋功能 | 支援 ?q= 查詢參數 | ⬜ 未開始 |
| B4-3 | 執行 RuboCop | 檢查程式碼風格 | ⬜ 未開始 |
| B4-4 | 測試列表 API | curl 或 RSpec 測試 | ⬜ 未開始 |

### Phase 5: 詳情 API (GET /todos/:id)
| 步驟 | 項目 | 說明 | 狀態 |
|------|------|------|------|
| B5-1 | 實作 show action | 取得單一 Todo | ⬜ 未開始 |
| B5-2 | 處理 404 錯誤 | 找不到時回傳錯誤 | ⬜ 未開始 |
| B5-3 | 執行 RuboCop | 檢查程式碼風格 | ⬜ 未開始 |
| B5-4 | 測試詳情 API | curl 或 RSpec 測試 | ⬜ 未開始 |
...
| 步驟 | 項目 | 說明 | 狀態 |
|------|------|------|------|
| B9-1 | 建立 Dockerfile | Backend Dockerfile | ⬜ 未開始 |
| B9-2 | 測試容器啟動 | 確認容器內可正常運作 | ⬜ 未開始 |
